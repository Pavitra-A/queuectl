...
This project focuses on **simplicity, reliability, and extensibility** — for building scalable background job systems.
...
| File | Purpose |
|------|----------|
| **`queuectl/cli.py`** | The entry point for all **CLI commands**. Handles initialization (`init`), enqueueing new jobs (`enqueue`), running workers (`worker`), listing jobs, and managing the DLQ (`dlq list`, `dlq retry`). |
...
| **`requirements.txt`** | Lists the external dependencies (currently only `typer`). |
...
| `available_at` | DATETIME | Time when job becomes available (for delayed jobs) |
...
| `original_job_id` | INTEGER | Reference to the original job |
